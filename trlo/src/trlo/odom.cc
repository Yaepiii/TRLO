...
std::atomic<bool> trlo::OdomNode::abort_(false);
...
float NormalizationYawAngle(float angle)
{
    if (angle > M_PI)
        angle -= 2 * M_PI;
    else if (angle < -M_PI)
...
float NormalizationRollPitchAngle(float angle)
{
    if (angle > M_PI / 2)
        angle -= M_PI;
...
void NormalizationYPR(Eigen::Vector3f& ypr) {
  if (ypr(0) > M_PI)
    ypr(0) -= 2 * M_PI;
  else if (ypr(0) < -M_PI)
    ypr(0) += 2 * M_PI;
  ...
  if (ypr(1) > M_PI / 2)
    ypr(1) -= M_PI;
  else if (ypr(1) < -M_PI / 2)
    ypr(1) += M_PI;

  if (ypr(2) > M_PI / 2)
    ypr(2) -= M_PI;
  else if (ypr(2) < -M_PI / 2)
    ypr(2) += M_PI;
}


bool comp(std::pair<int, double> i, std::pair<int, double> j) {return i.second < j.second;}

/**
 * Constructor
 **/

trlo::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {

  this->getParams();
  this->InitParam();
  this->allocateMemory();

  this->icp_sub = this->nh.subscribe("pointcloud", 1, &trlo::OdomNode::icpCB, this);
  this->imu_sub = this->nh.subscribe("imu", 1, &trlo::OdomNode::imuCB, this);
  this->box_sub = this->nh.subscribe("box", 1, &trlo::OdomNode::boxCB, this);

  this->odom_pub = this->nh.advertise<nav_msgs::Odometry>("odom", 1);
  this->trajectory_pub = this->nh.advertise<nav_msgs::Path>("trajectory", 1);
  ...
  this->robot_pub = this->nh.advertise<visualization_msgs::Marker>("robot", 10);

  this->save_traj_srv = this->nh.advertiseService("save_traj", &trlo::OdomNode::saveTrajectory, this);
  ...
  ROS_INFO("TRLO Odom Node Initialized");
...
void trlo::OdomNode::getParams() {

  // Version
  ros::param::param<std::string>("~trlo/version", this->version_, "0.0.0");

  // Frames
  ros::param::param<std::string>("~trlo/odomNode/odom_frame", this->odom_frame, "odom");
  ros::param::param<std::string>("~trlo/odomNode/child_frame", this->child_frame, "base_link");
  ...
  // Gravity alignment
  ros::param::param<bool>("~trlo/gravityAlign", this->gravity_align_, false);

  // Keyframe Threshold
  ros::param::param<double>("~trlo/odomNode/keyframe/threshD", this->keyframe_thresh_dist_, 0.1);
  ros::param::param<double>("~trlo/odomNode/keyframe/threshR", this->keyframe_thresh_rot_, 1.0);

  // Submap
  ros::param::param<int>("~trlo/odomNode/submap/keyframe/knn", this->submap_knn_, 10);
  ros::param::param<int>("~trlo/odomNode/submap/keyframe/kcv", this->submap_kcv_, 10);
  ros::param::param<int>("~trlo/odomNode/submap/keyframe/kcc", this->submap_kcc_, 10);

  // Initial Position
  ros::param::param<bool>("~trlo/odomNode/initialPose/use", this->initial_pose_use_, false);

  double px, py, pz, qx, qy, qz, qw;
  ros::param::param<double>("~trlo/odomNode/initialPose/position/x", px, 0.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/position/y", py, 0.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/position/z", pz, 0.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/orientation/w", qw, 1.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/orientation/x", qx, 0.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/orientation/y", qy, 0.0);
  ros::param::param<double>("~trlo/odomNode/initialPose/orientation/z", qz, 0.0);
  this->initial_position_ = Eigen::Vector3f(px, py, pz);
  this->initial_orientation_ = Eigen::Quaternionf(qw, qx, qy, qz);

  // Crop Box Filter
  ros::param::param<bool>("~trlo/odomNode/preprocessing/cropBoxFilter/use", this->crop_use_, false);
  ros::param::param<double>("~trlo/odomNode/preprocessing/cropBoxFilter/size", this->crop_size_, 1.0);

  // Voxel Grid Filter
  ros::param::param<bool>("~trlo/odomNode/preprocessing/voxelFilter/scan/use", this->vf_scan_use_, true);
  ros::param::param<double>("~trlo/odomNode/preprocessing/voxelFilter/scan/res", this->vf_scan_res_, 0.05);
  ros::param::param<bool>("~trlo/odomNode/preprocessing/voxelFilter/submap/use", this->vf_submap_use_, false);
  ros::param::param<double>("~trlo/odomNode/preprocessing/voxelFilter/submap/res", this->vf_submap_res_, 0.1);

  // Adaptive Parameters
  ros::param::param<bool>("~trlo/adaptiveParams", this->adaptive_params_use_, false);

  // IMU
  ros::param::param<bool>("~trlo/imu", this->imu_use_, false);
  ros::param::param<int>("~trlo/odomNode/imu/calibTime", this->imu_calib_time_, 3);
  ros::param::param<int>("~trlo/odomNode/imu/bufferSize", this->imu_buffer_size_, 2000);

  // BBox
  ros::param::param<int>("~trlo/odomNode/box/bufferSize", this->box_buffer_size_, 3);

  // Ground Contrain
  ros::param::param<bool>("~/trlo/ground", this->ground_use_, true);
  ros::param::param<double>("~trlo/odomNode/ground/threshold", this->ground_threshold_, 0.2);

  // GICP
  ros::param::param<int>("~trlo/odomNode/gicp/minNumPoints", this->gicp_min_num_points_, 100);
  ros::param::param<int>("~trlo/odomNode/gicp/s2s/kCorrespondences", this->gicps2s_k_correspondences_, 20);
  ros::param::param<double>("~trlo/odomNode/gicp/s2s/maxCorrespondenceDistance", this->gicps2s_max_corr_dist_, std::sqrt(std::numeric_limits<double>::max()));
  ros::param::param<int>("~trlo/odomNode/gicp/s2s/maxIterations", this->gicps2s_max_iter_, 64);
  ros::param::param<double>("~trlo/odomNode/gicp/s2s/transformationEpsilon", this->gicps2s_transformation_ep_, 0.0005);
  ros::param::param<double>("~trlo/odomNode/gicp/s2s/euclideanFitnessEpsilon", this->gicps2s_euclidean_fitness_ep_, -std::numeric_limits<double>::max());
  ros::param::param<int>("~trlo/odomNode/gicp/s2s/ransac/iterations", this->gicps2s_ransac_iter_, 0);
  ros::param::param<double>("~trlo/odomNode/gicp/s2s/ransac/outlierRejectionThresh", this->gicps2s_ransac_inlier_thresh_, 0.05);
  ros::param::param<int>("~trlo/odomNode/gicp/s2m/kCorrespondences", this->gicps2m_k_correspondences_, 20);
  ros::param::param<double>("~trlo/odomNode/gicp/s2m/maxCorrespondenceDistance", this->gicps2m_max_corr_dist_, std::sqrt(std::numeric_limits<double>::max()));
  ros::param::param<int>("~trlo/odomNode/gicp/s2m/maxIterations", this->gicps2m_max_iter_, 64);
  ros::param::param<double>("~trlo/odomNode/gicp/s2m/transformationEpsilon", this->gicps2m_transformation_ep_, 0.0005);
  ros::param::param<double>("~trlo/odomNode/gicp/s2m/euclideanFitnessEpsilon", this->gicps2m_euclidean_fitness_ep_, -std::numeric_limits<double>::max());
  ros::param::param<int>("~trlo/odomNode/gicp/s2m/ransac/iterations", this->gicps2m_ransac_iter_, 0);
  ros::param::param<double>("~trlo/odomNode/gicp/s2m/ransac/outlierRejectionThresh", this->gicps2m_ransac_inlier_thresh_, 0.05);
...
void trlo::OdomNode::InitParam() {
  ...
  this->box_buffer.set_capacity(this->box_buffer_size_);
  ...
  this->ground_normal = Eigen::Vector3f(0., 0., 0.);
}

/**
 * Allocate Memory
*/
void trlo::OdomNode::allocateMemory()
{
...
void trlo::OdomNode::start() {
  ROS_INFO("Starting TRLO Node");

  printf("\033[2J\033[1;1H");
  std::cout << std::endl << "==== TRLO v" << this->version_ << " ====" << std::endl << std::endl;
...
void trlo::OdomNode::stop() {
  ROS_WARN("Stopping TRLO Node");
...
void trlo::OdomNode::abortTimerCB(const ros::TimerEvent& e) {
  if (abort_) {
    stop();
  }
}
...
void trlo::OdomNode::publishPose() {
 ...
 * Publish Trajectory
 **/
void trlo::OdomNode::publishTrajectory()
{
  geometry_msgs::PoseStamped pose_curr;
  pose_curr.header.stamp = this->scan_stamp;
  pose_curr.header.frame_id = this->odom_frame;
  pose_curr.pose.position.x = this->pose[0];
  pose_curr.pose.position.y = this->pose[1];
  pose_curr.pose.position.z = this->pose[2];
  pose_curr.pose.orientation.w = this->rotq.w();
  pose_curr.pose.orientation.x = this->rotq.x();
  pose_curr.pose.orientation.y = this->rotq.y();
  pose_curr.pose.orientation.z = this->rotq.z();

  this->robot_trajectory.header.stamp = this->scan_stamp;
  this->robot_trajectory.header.frame_id = this->odom_frame;
  this->robot_trajectory.poses.push_back(pose_curr);
  this->trajectory_pub.publish(this->robot_trajectory);
}
...
void trlo::OdomNode::publishTransform() {
...
void trlo::OdomNode::publishKeyframe() {

  // Publish keyframe pose
  this->kf.header.stamp = this->scan_stamp;
  this->kf.header.frame_id = this->odom_frame;  // odom
  this->kf.child_frame_id = this->child_frame;  // base_link
 ...
 **/
void trlo::OdomNode::publishRobot()
{
    visualization_msgs::Marker tempMarker;
    tempMarker.id = 0;

    tempMarker.header.frame_id = this->odom_frame;
    tempMarker.header.stamp = this->scan_stamp;
    tempMarker.id = 0;
    tempMarker.ns = "robot";
    tempMarker.type = visualization_msgs::Marker::CUBE;
    tempMarker.action = visualization_msgs::Marker::ADD;

    tempMarker.pose.position.x = this->pose[0];
    tempMarker.pose.position.y = this->pose[1];
    tempMarker.pose.position.z = this->pose[2];
    tempMarker.pose.orientation.x = this->rotq.x();
    tempMarker.pose.orientation.y = this->rotq.y();
    tempMarker.pose.orientation.z = this->rotq.z();
    tempMarker.pose.orientation.w = this->rotq.w();

    tempMarker.scale.x = 3.6;
    ...
    this->robot_pub.publish(tempMarker);
}
...
void trlo::OdomNode::preprocessPoints() {

  // Remove NaNs
  std::vector<int> idx;
  this->current_scan->is_dense = false;
  pcl::removeNaNFromPointCloud(*this->current_scan, *this->current_scan, idx);
  //this->removeClosedPointCloud(*this->current_scan, *this->current_scan, 0.5, 80);
...
void trlo::OdomNode::removeClosedPointCloud(const pcl::PointCloud<pcl::PointXYZI> &cloud_in, pcl::PointCloud<pcl::PointXYZI> &cloud_out, float th1, float th2)
{
...
void trlo::OdomNode::initializeInputTarget() {
  ...
  *this->keyframe_cloud = *first_keyframe;
  ...
  this->publish_keyframe_thread = std::thread( &trlo::OdomNode::publishKeyframe, this );
...
void trlo::OdomNode::setInputSources(){
...
void trlo::OdomNode::gravityAlign() {
...
void trlo::OdomNode::initializeTRLO() {
  ...
  this->trlo_initialized = true;
  std::cout << "TRLO initialized! Starting localization..." << std::endl;
...
void trlo::OdomNode::icpCB(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  // TRLO Initialization procedures (IMU calib, gravity align)
  if (!this->trlo_initialized) {
    this->initializeTRLO();
    return;
  }
  ...
  // Compute Metrics
  this->metrics_thread = std::thread( &trlo::OdomNode::computeMetrics, this );
  ...
  this->publish_thread = std::thread( &trlo::OdomNode::publishToROS, this );
  this->publish_thread.detach();

  // Debug statements and publish custom trlo message
  this->debug_thread = std::thread( &trlo::OdomNode::debug, this );
...
void trlo::OdomNode::imuCB(const sensor_msgs::Imu::ConstPtr& imu) {
...
void trlo::OdomNode::boxCB(const jsk_recognition_msgs::BoundingBoxArrayPtr& box) {
  this->mtx_box.lock();
  this->box_buffer.push_front(*box);
  this->mtx_box.unlock();
}
...
void trlo::OdomNode::getNextPose() {
  ...
  if (!this->box_buffer.empty())
  {
    double cur_box_z = 0;
    double pre_box_z = 0;
    std::vector<Eigen::Vector3f> groundPoints;
    for (auto box : this->box_buffer.front().boxes) {
      cur_box_z += box.pose.position.z;

      Eigen::Vector3f gp;
      gp[0] = box.pose.position.x;
      gp[1] = box.pose.position.y;
      gp[2] = box.pose.position.z - box.dimensions.z / 2;
      groundPoints.push_back(gp);
    }
    cur_box_z /= this->box_buffer.front().boxes.size();
    for (auto box : this->box_buffer[1].boxes) {
      pre_box_z += box.pose.position.z;
    ...
    pre_box_z /= this->box_buffer[1].boxes.size();


    // extract ground normal
    int groundPointsSize = groundPoints.size();
    ...
    for (int i = 0; i < groundPointsSize; i++) {
      matA0(i, 0) = groundPoints[i](0);
      matA0(i, 1) = groundPoints[i](1);
      matA0(i, 2) = groundPoints[i](2);
    }

    // find the norm of plane
    this->ground_normal = matA0.colPivHouseholderQr().solve(matB0);
    this->ground_normal = this->T.block<3,3>(0,0) * this->ground_normal + this->T.block<3,1>(0,3);
    float negative_OA_dot_norm = 1 / this->ground_normal.norm();
    this->ground_normal.normalize();

    // calculate angle between the two vectors
    Eigen::Vector3f grav(0, 0, 1);
    Eigen::Quaternionf ground_q = Eigen::Quaternionf::FromTwoVectors(this->ground_normal, grav);
    ground_q.normalize();
    ...
    for (int i = 0; i < groundPointsSize; i++)
    {
      ...
      if (fabs(this->ground_normal(0) * groundPoints[i](0) +
            this->ground_normal(1) * groundPoints[i](1) +
            this->ground_normal(2) * groundPoints[i](2) + negative_OA_dot_norm) > this->ground_threshold_)
      {
        planeValid = false;
        break;
      }
    ...
    this->para_tz[0] = static_cast<double>(this->T(2,3));
    Eigen::Quaternionf q_tem(this->T.block<3,3>(0,0));
    q_tem.normalize();
    this->para_q[0] = static_cast<double>(q_tem.x());
    this->para_q[1] = static_cast<double>(q_tem.y());
    this->para_q[2] = static_cast<double>(q_tem.z());
    this->para_q[3] = static_cast<double>(q_tem.w());

    ceres::LossFunction *loss_function = new ceres::HuberLoss(0.1);
    ceres::Problem::Options problem_options;

    ceres::Problem problem(problem_options);
    problem.AddParameterBlock(this->para_tz, 1);
    problem.AddParameterBlock(this->para_q, 4);

    if (this->ground_use_ && !this->box_buffer.empty()) {
      // constraint
      if (planeValid)
      {
        ceres::CostFunction* Gravity_factor = PitchRollFactor::Create(ground_ypr[1], ground_ypr[2], 0.02);
        problem.AddResidualBlock(Gravity_factor, NULL, this->para_q);
      }

      double ground_cov = 40000;
      Eigen::Vector3f ypr = R2ypr(Eigen::Matrix3f(this->T.block<3,3>(0,0)));
      if ((cur_box_z - pre_box_z) < 0.5) {
        ground_cov = 0.01;  // 0.0001
      }
      if ((fabs(ypr.y()) < 5.0) && ((cur_box_z - pre_box_z) < this->ground_threshold_))
      {
        ground_cov = 0.00001;  // 0.000001
      }
                
      ceres::CostFunction* Ground_factor = GroundFactor::Create(ground_cov, this->para_tz_pre);
      problem.AddResidualBlock(Ground_factor, NULL, this->para_tz);
    }
    ...
    this->ground_optimize_times.push_back(ros::Time::now().toSec() - tg);

    this->T(2,3) = static_cast<float>(this->para_tz[0]);
    Eigen::Vector3f raw_rpy = q_tem.toRotationMatrix().eulerAngles(2, 1, 0);
    q_tem.w() = static_cast<float>(this->para_q[3]);
    q_tem.x() = static_cast<float>(this->para_q[0]);
    q_tem.y() = static_cast<float>(this->para_q[1]);
    q_tem.z() = static_cast<float>(this->para_q[2]);
    q_tem.normalize();
    Eigen::Vector3f opt_rpy = q_tem.toRotationMatrix().eulerAngles(2, 1, 0);
    raw_rpy[1] = opt_rpy[1]; //pitch
    raw_rpy[2] = opt_rpy[2]; //roll
    
    // this->T.block<3,3>(0,0) = ypr2R(raw_rpy);
    
    para_tz_pre = this->T(2,3);
  }
...
void trlo::OdomNode::integrateIMU() {
...
void trlo::OdomNode::propagateS2S(Eigen::Matrix4f T) {
...
void trlo::OdomNode::propagateS2M() {
...
void trlo::OdomNode::transformCurrentScan() {
  this->current_scan_t = pcl::PointCloud<PointType>::Ptr (new pcl::PointCloud<PointType>);
...
void trlo::OdomNode::computeMetrics() {
...
void trlo::OdomNode::computeSpaciousness() {
...
void trlo::OdomNode::computeConvexHull() {
...
void trlo::OdomNode::computeConcaveHull() {
...
void trlo::OdomNode::updateKeyframes() {
...
void trlo::OdomNode::setAdaptiveParams() {
 ...
 **/
void trlo::OdomNode::pushSubmapIndices(std::vector<float> dists, int k, std::vector<int> frames) {
  ...
  for (int i = 0; i < dists.size(); ++i) {
    if (dists[i] <= kth_element)
      ...
      this->submap_kf_idx_hash.insert(frames[i]);
  }
...
void trlo::OdomNode::getSubmapKeyframes() {

  double submap_build_time = ros::Time::now().toSec();

  // clear vector of keyframe indices to use for submap
  this->submap_kf_idx_curr.clear();
  this->submap_kf_idx_hash.clear();
  ...
  for (auto i : this->submap_kf_idx_hash){
  ...
  this->submap_build_times.push_back(ros::Time::now().toSec() - submap_build_time);

}

bool trlo::OdomNode::saveTrajectory(trlo::save_traj::Request& req,
...
void trlo::OdomNode::debug() {
  ...
  double avg_submap_build_time = std::accumulate(this->submap_build_times.begin(), this->submap_build_times.end(), 0.0) / this->submap_build_times.size();
  double avg_ground_optimize_time = std::accumulate(this->ground_optimize_times.begin(), this->ground_optimize_times.end(), 0.0) / this->ground_optimize_times.size();
  ...
  std::cout << std::endl << "==== TRLO v" << this->version_ << " ====" << std::endl;
  ...
  std::cout << "Submap build Time :: " << std::setfill(' ') << std::setw(6) << this->submap_build_times.back()*1000. << " ms    // Avg: " << std::setw(5) << avg_submap_build_time*1000. << std::endl;
  std::cout << "Ground optimize Time :: " << std::setfill(' ') << std::setw(6) << this->ground_optimize_times.back()*1000. << " ms    // Avg: " << std::setw(5) << avg_ground_optimize_time*1000. << std::endl;

  std::cout << "concave size is: " << this->keyframe_concave.size() << std::endl;
  std::cout << "this->submap_kf_idx_hash size is: " << this->submap_kf_idx_hash.size() << std::endl;
}
